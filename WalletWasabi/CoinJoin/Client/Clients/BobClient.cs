...
namespace WalletWasabi.CoinJoin.Client.Clients
{
	public class BobClient : TorDisposableBase
	{
		...
		public async Task<bool> PostOutputAsync(long roundId, ActiveOutput activeOutput)
		{
			...
			using var response = await TorClient.SendAsync(HttpMethod.Post, $"/api/v{MustardWalletLTCClient.ApiVersion}/ltc/chaumiancoinjoin/output?roundId={roundId}", request.ToHttpStringContent()).ConfigureAwait(false);
